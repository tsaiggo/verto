...
function routedFetch(
  routes: Array<[RegExp, (url: string) => Response]>,
): { fetchImpl: FetchLike; calls: string[]; headers: HeadersInit[] } {
  const calls: string[] = [];
...
describe("fetchUser", () => {
  it("returns the user's login, name and avatar", async () => {
    ...
    // Token is sent as a ******
    expect(
      (headers[0] as Record<string, string>).Authorization,
    ).toBe("Bearer " + "tok");
  });
...
describe("listRepos", () => {
  it("flattens paginated results and stops on a short page", async () => {
    ...
    const page2 = [
      { full_name: "octo/last", default_branch: "dev", private: false },
    ];
...
describe("validateContentPath", () => {
  ...
  it("returns true for the repo root (empty path)", async () => {
    ...
    expect(await validateContentPath("tok", "octo/repo", "main", "", fetchImpl)).toBe(
      true,
    );
  ...
  it("returns true when the path exists in the tree", async () => {
    ...
    expect(
      await validateContentPath("tok", "octo/repo", "main", "/content", fetchImpl),
    ).toBe(true);
  });
  ...
  it("returns false when the path is absent", async () => {
    const { fetchImpl } = routedFetch([[/git\/trees/, () => jsonResponse(tree)]]);
    expect(
      await validateContentPath("tok", "octo/repo", "main", "docs", fetchImpl),
    ).toBe(false);
  });
});
