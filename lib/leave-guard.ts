...
type UnsavedSaveStatus = "idle" | "saving" | "saved" | "conflict" | "error";

const LEAVE_UNSAVED_MESSAGE = "You have unsaved changes. Leave this page and discard this draft?";
const HISTORY_GUARD_STATE_KEY = "__vertoUnsavedLeaveGuard";
const CONFIRMED_NAVIGATION_WINDOW_MS = 1_000;

interface LeaveNavigationEvent extends Event {
...
export function shouldBlockUnsavedLeave(
  source: string,
  baselineSource: string,
  saveStatus: UnsavedSaveStatus
): boolean {
...
function createLeaveGuardSession(navigation: EventTarget | null): LeaveGuardSession {
  ...
  const session: LeaveGuardSession = {
    token: `leave-${historyGuardSequence}`,
...
export function useUnsavedLeaveGuard(shouldBlockLeave: boolean): void {
  ...
  useEffect(() => {
    ...
    const handleDocumentClick = (event: MouseEvent) => {
      ...
      if (!window.confirm(LEAVE_UNSAVED_MESSAGE)) {
        event.preventDefault();
        return;
      }
    ...
    const handleNavigate = (event: Event) => {
      const navigateEvent = event as LeaveNavigationEvent;
