...
describe('Shiki code block colors', () => {
  it('maps both light and dark Shiki CSS variables in global styles', async () => {
    ...
    const lightTokenRule = css.match(/\.shiki,\s*\.shiki span\s*{(?<body>[^}]*)}/s)?.groups?.body ?? '';
    const darkTokenRule = css.match(/html\.dark \.shiki,\s*html\.dark \.shiki span\s*{(?<body>[^}]*)}/s)?.groups?.body ?? '';
    ...
    expect(css).toMatch(/\.shiki,\s*\.shiki span\s*{[^}]*font-style:\s*var\(--shiki-light-font-style\)/s);
    expect(darkTokenRule).toMatch(/color:\s*var\(--shiki-dark\)/);
    expect(darkTokenRule).not.toMatch(/background-color:/);
    expect(css).toMatch(/html\.dark \.shiki,\s*html\.dark \.shiki span\s*{[^}]*font-style:\s*var\(--shiki-dark-font-style\)/s);
  });
