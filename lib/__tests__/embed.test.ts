...
function mockFetch(handlers: Array<[RegExp | string, (url: string) => unknown]>) {
  return vi.fn(async (input: string | URL | Request) => {
    ...
    for (const [match, handler] of handlers) {
      const ok = typeof match === "string" ? u.includes(match) : match.test(u);
      if (ok) {
        const payload = handler(u);
        if (payload && typeof payload === "object" && "status" in payload) {
          const p = payload as { status: number; body?: unknown; text?: string };
          return new Response(p.text !== undefined ? p.text : JSON.stringify(p.body ?? {}), {
            status: p.status,
...
describe("resolveEmbed", () => {
  ...
  it("resolves a YouTube watch URL via oEmbed", async () => {
    ...
    const meta = await resolveEmbed("https://www.youtube.com/watch?v=dQw4w9WgXcQ");
  ...
  it("handles youtu.be shortener URLs", async () => {
    vi.stubGlobal("fetch", mockFetch([["youtube.com/oembed", () => ({ title: "short" })]]));
  ...
  it("falls back to OpenGraph for unknown URLs", async () => {
    ...
    vi.stubGlobal("fetch", mockFetch([["example.com", () => ({ status: 200, text: html })]]));
