...
function TabCarrier(_props: { label: string; value?: string; children?: React.ReactNode }) {
  void _props;
  return null;
}
...
describe("Card / CardGroup", () => {
  ...
  it("passes cols through as a CSS variable", () => {
    ...
    const html = renderToStaticMarkup(createElement(CardGroup, props, null));
...
describe("MDX compound components", () => {
  it("collects Accordion-shaped children without relying on component identity", () => {
    const html = renderToStaticMarkup(
      createElement(
        AccordionGroup,
        null,
        createElement(AccordionCarrier, { title: "First panel", defaultOpen: true }, "First body"),
  ...
  it("collects Tab-shaped children without relying on component identity", () => {
    ...
    const html = renderToStaticMarkup(createElement(Tabs, props, install, usage));
