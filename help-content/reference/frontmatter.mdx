...
These fields apply to Library documents and bundled Help documents. RSS and
Atom items use feed metadata in Inbox instead of this frontmatter contract. A
connected local folder and a build-time local or OneDrive source use the same
field parser.
...
The OneDrive build-time source does not reliably expose a per-file modification
time. For deterministic ordering across local and OneDrive builds, set
`date`, `updated`, or `order` explicitly instead of relying only on source
mtime.
</Callout>
...
- `status` — a single workflow label, such as `draft`, `reading`,
  `evergreen`, or `published`; surfaced in Library metadata and linked to
  `/read/status/<status>` filtered views.
- `cover` — an image URL or path used for page and Library previews.
