---
title: "Block Components"
description: "The full MDX component catalog — Callouts, Toggles, Cards, Tabs, Steps, Accordions, FileTrees, and more"
order: 2
---

# Block Components

Verto ships a rich set of MDX components for building content beyond plain Markdown. Some are automatic upgrades to standard Markdown syntax — tables, blockquotes, code blocks, and inline code all get enhanced styling with zero extra effort. Others are custom JSX tags you write directly, from simple Callouts to documentation-grade Tabs, Steps, Cards, Accordions, and FileTrees. None require an import — they're registered globally in `mdx-components.tsx`.

<Callout type="tip">
Every component on this page works in Library and Help documents. Just write the syntax shown below.
</Callout>

## Available Components

| Component | Syntax | Description |
|-----------|--------|-------------|
| Callout | `<Callout>` JSX tag | Admonition boxes for info, warnings, and tips |
| Toggle | `<Toggle>` JSX tag | Collapsible content sections |
| BookmarkCard | `<BookmarkCard>` JSX tag | Link preview card with title and description |
| Figure | `<Figure>` JSX tag | Image with optional caption |
| DiagramPlaceholder | `<DiagramPlaceholder>` JSX tag | Layout-only marker for an intentionally omitted diagram |
| TaskList | `- [x]` / `- [ ]` checkboxes | Checkbox task lists via GFM syntax |
| Table | `\| Header \|` pipe syntax | Auto-styled Markdown tables |
| BlockquoteStyled | `>` blockquote syntax | Auto-styled blockquotes |
| CodeBlock | ` ``` ` fenced code blocks | Syntax-highlighted code with copy button |
| InlineCode | `` `backticks` `` | Styled inline code spans |
| Tabs | `<Tabs>` / `<Tab>` JSX tags | Grouped variants with URL-hash sync |
| Steps | `<Steps>` JSX tag | Auto-numbered step sequences |
| Cards | `<CardGroup>` / `<Card>` JSX tags | Grid of (optionally linked) cards |
| Accordion | `<AccordionGroup>` / `<Accordion>` JSX tags | Expandable Q&A / FAQ panels |
| FileTree | `<FileTree>` / `<Folder>` / `<File>` | Project-layout file trees |

## Callout

Highlighted admonition boxes. Three variants: `info` (default), `warning`, and `tip`.

**Props:**
- `type` (optional): `'info'` | `'warning'` | `'tip'`. Defaults to `'info'`.

```mdx
<Callout>
This is an info callout. The default type.
</Callout>

<Callout type="warning">
Something the reader should be careful about.
</Callout>

<Callout type="tip">
A helpful suggestion.
</Callout>
```

## Toggle

Collapsible content block. Click the title to expand or collapse.

**Props:**
- `title` (required): string displayed as the toggle header.

```mdx
<Toggle title="Click to expand">
Hidden content goes here. Supports full Markdown inside.
</Toggle>
```

## BookmarkCard

A styled link preview card. Useful for referencing external resources or other pages.

**Props:**
- `url` (required): the link URL.
- `title` (required): display title for the card.
- `description` (optional): short summary text.

```mdx
<BookmarkCard
  url="https://github.com/tsaiggo/verto"
  title="Verto on GitHub"
  description="Source code and documentation."
/>
```

## Figure

Image display with an optional caption underneath.

**Props:**
- `src` (required): image path or URL.
- `alt` (required): alt text for accessibility.
- `caption` (optional): caption displayed below the image.

```mdx
<Figure
  src="/images/example.png"
  alt="Screenshot of the dashboard"
  caption="The main dashboard view."
/>
```

## DiagramPlaceholder

A layout-only marker for a diagram you intentionally have not embedded. It
renders a styled box with a title and optional description; it does not parse
or render diagram source. For working diagrams, use Mermaid, Excalidraw, or D2
as documented in [Diagrams](/help/writing/diagrams).

**Props:**
- `title` (required): name of the planned diagram.
- `description` (optional): what the diagram will show.

```mdx
<DiagramPlaceholder
  title="Architecture Overview"
  description="Shows the MDX pipeline from content to static HTML."
/>
```

## TaskList

Checkbox task lists using standard GFM syntax. No special JSX needed.

```mdx
- [x] Write documentation
- [x] Add code examples
- [ ] Review and publish
```

## Table

Standard Markdown pipe tables, automatically styled by Verto. No extra syntax required.

```mdx
| Feature | Status |
|---------|--------|
| Dark mode | ✅ Shipped |
| Blog system | ✅ Shipped |
| Search | 🚧 Planned |
```

## BlockquoteStyled

Standard Markdown blockquotes with enhanced styling. Just use the `>` prefix as usual.

```mdx
> Good documentation is written for the reader, not the author.
```

## CodeBlock

Fenced code blocks with a language tag get Shiki-powered syntax highlighting, a language label, and a copy-to-clipboard button. Highlighting is rendered at build time with dual light/dark themes, so there's zero client-side JavaScript.

````mdx
```typescript
function greet(name: string): string {
  return `Hello, ${name}!`;
}
```
````

For full details on supported languages and themes, see [Syntax Highlighting](/help/writing/syntax-highlighting).

## InlineCode

Wrap text in backticks for styled inline code. Works exactly like standard Markdown.

```mdx
Use the `npm run dev` command to start the dev server.
```

---

Beyond the core blocks above, Verto ships a small set of MDX components designed for documentation-style content.

## Tabs

Use `<Tabs>` to group related variants — package managers, OS flavours, code dialects.

<Tabs id="install">
  <Tab label="npm">
    ```bash
    npm install
    ```
  </Tab>
  <Tab label="pnpm">
    ```bash
    pnpm install
    ```
  </Tab>
  <Tab label="yarn">
    ```bash
    yarn install
    ```
  </Tab>
</Tabs>

When `id="install"` is set, the active tab syncs with the URL hash — share `…#install=pnpm` and your reader lands on the right tab. Arrow keys navigate the tablist for accessibility.

## Steps

`<Steps>` numbers heading-led blocks automatically. Pure CSS counter, zero JS:

<Steps>
### Drop in your files

Put any `.md` or `.mdx` files under `content/`.

### Run the dev server

```bash
npm run dev
```

### Open `/read`

Verto auto-discovers everything and serves it under the reader.
</Steps>

## Cards

Use `<CardGroup cols={n}>` to lay out multiple `<Card>` entries. Cards with `href` become clickable.

<CardGroup cols={2}>
  <Card title="Quick Start" description="Spin up Verto in 60 seconds" href="/help/getting-started/installation" />
  <Card title="Inline Comments" description="Annotate your prose with [^c-1] popups" href="/help/core-concepts/inline-comments" />
  <Card title="Syntax Highlighting" description="Shiki + dual themes + diff/focus/word" href="/help/writing/syntax-highlighting" />
  <Card title="Dark Mode" description="CSS-variable based, no flash" href="/help/reading-experience/dark-mode" />
</CardGroup>

## Accordion

Single panel, or grouped — pass `exclusive` to enforce one-open-at-a-time:

<AccordionGroup exclusive>
  <Accordion title="Why footnote-style inline comments?">
    Because `[^c-id]` is valid CommonMark syntax. GitHub, Typora, and any other Markdown renderer fall back to a regular footnote — your annotations are never lost.
  </Accordion>
  <Accordion title="Does Verto need a database?">
    No. The build-time source is pre-rendered, while a runtime local library is read from the current device. Neither path requires a content database.
  </Accordion>
  <Accordion title="Can I customize the navigation order?">
    Yes — drop a `content/navigation.json` with `overrides` keyed by slug. See [Navigation](/help/core-concepts/navigation).
  </Accordion>
</AccordionGroup>

## FileTree

For documentation that needs to show project layouts:

<FileTree>
  <Folder name="content" defaultOpen>
    <Folder name="docs">
      <Folder name="getting-started">
        <File name="introduction.mdx" />
      </Folder>
      <Folder name="writing">
        <File name="syntax-highlighting.mdx" />
        <File name="block-components.mdx" comment="this page" />
      </Folder>
    </Folder>
    <File name="navigation.json" comment="optional overrides" />
  </Folder>
  <Folder name="components/mdx" defaultOpen={false}>
    <File name="Tabs.tsx" />
    <File name="Card.tsx" />
  </Folder>
</FileTree>

## Related

- [MDX Authoring](/help/writing/mdx-authoring) for the full MDX writing guide
- [Syntax Highlighting](/help/writing/syntax-highlighting) for code block details and supported languages
- [Inline Comments](/help/core-concepts/inline-comments) — author asides with `[^c-N]` popups
